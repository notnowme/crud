...
export const boardGetAll: RequestHandler = async (req, res) => {
    try {
        ...
        const allCounts = await db.qna_board.count();
        const pageSize = 20;
        ...
        console.error(`[GET] /api/board/qna`, err);
...
export const boardGetOne: RequestHandler = async(req, res) => {
    try {
        ...
        const board = await db.qna_board.findFirst({
            where: {
                no: parseInt(no)
            },
            include: {
        ...
        console.error(`[GET] /api/board/qna/{no}`, err);
        return res.status(500).json({ok: false, message: 'Internel Server Error'});
    }
}
...
export const boardWrite: RequestHandler = async(req, res) => {
    try {
        ...
        const board = await db.qna_board.create({
            data: {
                author_no: no,
                title,
                content,
        ...
        console.log(`[POST] /api/board/qna`, err);
        return res.status(500).json({ok: false, message: 'Internel Server Error'});
    }
};
...
export const boardModify: RequestHandler = async (req, res) => {
    try {
        ...
        const modifiedBoard = await db.qna_board.update({
            where: {
                no: parseInt(boardNo)
            },
            data: {
        ...
        console.error(`[PATCH] /api/board/qna/{no}`, err);
        return res.status(500).json({ok: false, message: 'Internel Server Error'});
    }
};
...
export const boardDelete: RequestHandler = async (req, res) => {
    try {
        ...
        console.error(`[DELETE] /api/board/qna/{no}`, err);
        return res.status(500).json({ok: false, message: 'Internel Server Error'});
    }
}